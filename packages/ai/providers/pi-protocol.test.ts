...
describe('Pi RPC protocol schemas', () => {
  ...
  test('recognizes response envelopes when the payload is malformed', () => {
    expect(Schema.decodeUnknownSync(PiResponseEnvelopeSchema)({
      type: 'response',
      id: 'request-4',
  ...
  test('decodes error responses and rejects malformed envelopes', () => {
    ...
    expect(
      Option.getOrUndefined(Schema.decodeUnknownOption(PiResponseSchema)({
        type: 'response',
        id: 42,
        success: true,
      })),
  ...
  test('decodes model lists and rejects malformed model entries', () => {
    ...
    expect(
      Option.getOrUndefined(Schema.decodeUnknownOption(PiModelsResponseSchema)({
  ...
  test('decodes optional state and provider configuration payloads', () => {
    ...
    expect(Schema.decodeUnknownSync(PiSDKConfigSchema)({
      type: 'pi-sdk',
      cwd: '/tmp/repo',
