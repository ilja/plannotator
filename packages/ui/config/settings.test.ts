...
describe("UI server config decoding", () => {
  ...
  test("preserves valid empty and null conventional-label settings", () => {
    expect(
      SETTINGS.conventionalLabels.fromServer(decodeUiServerConfig({ conventionalLabels: [] })),
    ).toBe("[]");
    expect(
      SETTINGS.conventionalLabels.fromServer(decodeUiServerConfig({ conventionalLabels: null })),
    ).toBeNull();
  ...
  test("rejects legacy string booleans atomically during server synchronization", () => {
    expect(
      SETTINGS.conventionalLabels.toServer(
