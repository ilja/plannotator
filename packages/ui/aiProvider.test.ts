...
const providers: AIProviderOption[] = [
  ...
  },
  {
    ...
    models: [{ id: "fallback-default", label: "Fallback Default", default: true }],
  },
];
...
describe("saved AI provider preferences", () => {
  ...
  it("keeps recognized origin preferences and filters malformed siblings", () => {
    expect(
      decodeProviderByOrigin(JSON.stringify({ pi: "pi-local", unknown: "ignored", codex: 42 })),
    ).toEqual({
      pi: "pi-local",
    });
...
describe("AI provider origin defaults", () => {
  ...
  it("stores explicit Pi choices without changing the global fallback", () => {
    const next = applyAIProviderSelection(settings({ providerId: "fallback-local" }), {
      providerId: "pi-local",
      model: "pi-alt",
      origin: "pi",
    });
  ...
  it("does not store non-Pi provider choices", () => {
    const next = applyAIProviderSelection(settings({ providerId: "pi-local" }), {
