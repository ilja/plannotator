...
describe("loadEditorAnnotationsResponse", () => {
  test("loads valid responses and returns undefined for non-OK or invalid JSON", async () => {
    expect(
      await loadEditorAnnotationsResponse(new Response(JSON.stringify({ annotations: [] }))),
    ).toEqual([]);
    expect(
      await loadEditorAnnotationsResponse(
        new Response(JSON.stringify({ annotations: [] }), { status: 503 }),
      ),
    ).toBeUndefined();
