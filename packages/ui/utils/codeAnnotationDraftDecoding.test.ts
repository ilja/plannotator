...
describe("decodeSuccessfulCodeAnnotationDraft", () => {
  ...
  test("preserves valid draft generations and ignores malformed ones", () => {
    ...
    expect(
      decodeSuccessfulCodeAnnotationDraft({ draftGeneration: -1 })?.draftGeneration,
    ).toBeNull();
    expect(
      decodeSuccessfulCodeAnnotationDraft({ draftGeneration: 1.5 })?.draftGeneration,
    ).toBeNull();
    expect(
      decodeSuccessfulCodeAnnotationDraft({ draftGeneration: Number.POSITIVE_INFINITY })
        ?.draftGeneration,
    ).toBeNull();
    expect(
      decodeSuccessfulCodeAnnotationDraft({ draftGeneration: "4" })?.draftGeneration,
    ).toBeNull();
  });
});
...
describe("decodeMissingCodeAnnotationDraft", () => {
  test("decodes missing responses with and without a generation", () => {
    ...
    expect(decodeMissingCodeAnnotationDraft({ found: false, draftGeneration: 0 })).toEqual({
      draftGeneration: 0,
    });
    expect(decodeMissingCodeAnnotationDraft({ found: false, draftGeneration: 4 })).toEqual({
      draftGeneration: 4,
    });
  });

  test("ignores malformed generations without rejecting a missing response", () => {
    expect(decodeMissingCodeAnnotationDraft({ found: false, draftGeneration: -1 })).toEqual({
      draftGeneration: null,
    });
    expect(decodeMissingCodeAnnotationDraft({ found: false, draftGeneration: 1.5 })).toEqual({
      draftGeneration: null,
    });
    expect(decodeMissingCodeAnnotationDraft({ found: false, draftGeneration: "4" })).toEqual({
      draftGeneration: null,
    ...
    expect(
      decodeMissingCodeAnnotationDraft({ found: false, draftGeneration: Number.POSITIVE_INFINITY }),
    ).toEqual({ draftGeneration: null });
  });
