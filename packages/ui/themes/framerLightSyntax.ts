/**
 * Pure Framer Light syntax theme definition.
 * Based on https://github.com/balanceiskey/vim-framer-syntax by Sundeep Malladi (MIT).
 *
 * This module exports only the theme data. Provider-specific registration
 * (Shiki directly or via @pierre/diffs) happens in thin adapters:
 * - annotate mode: codeHighlighting layer
 * - review mode: review-editor/themes/framerLightSyntax.ts wrapper
 */

export const FRAMER_LIGHT_SYNTAX_THEME_NAME = "plannotator-framer-light";

export const framerLightSyntaxTheme = {
  ...
  tokenColors: [
    {
      ...
      settings: { foreground: "#bbbbbb", fontStyle: "italic" },
    },
    {
      scope: ["keyword", "keyword.control", "keyword.operator.new", "variable.other.property"],
      settings: { foreground: "#0099ff" },
    },
    {
      scope: [
        "storage",
    ...
    {
      scope: ["variable.language", "invalid", "invalid.illegal"],
      settings: { foreground: "#ff3377" },
    },
    {
      scope: ["meta.interpolation", "punctuation.section.embedded", "meta.annotation"],
      settings: { foreground: "#dd88ff" },
    },
  ],
} as const;
