...
describe("code-review annotation draft persistence", () => {
  test.skipIf(!hasDom)(
    "deleting every annotation removes the draft so it does not resurrect (#948)",
    async () => {
  ...
  test.skipIf(!hasDom)(
    "external (source-tagged) annotation churn does not arm engagement or delete the draft",
    async () => {
  ...
  test.skipIf(!hasDom)(
    "a fresh, unengaged session does not delete an unrestored draft on disk",
    async () => {
      ...
      const s = await mountSession(options());
      expect(s.result.current!.draftBanner).toEqual({
        count: 1,
        viewedCount: 0,
        timeAgo: "just now",
      });
