...
interface BridgeDocument {
  ...
  createTreeWalker: () => { currentNode: { textContent: string; length: number }; nextNode: () => boolean };
  createRange: () => { setStart: () => void; setEnd: () => void; surroundContents: (mark: BridgeElement) => void };
...
function createBridgeRuntime() {
  ...
  const parent: BridgeParent = {
    postMessage: (message: any) => { parentMessages.push(message); },
  };
  ...
  const createMark = (): BridgeElement => {
    ...
    return {
      ...
      set className(value: string) { className = value; },
      classList: {
        add: (...classes) => {
        ...
        remove: (...classes) => {
          className = className.split(' ').filter((value) => !classes.includes(value)).join(' ');
          if (classes.includes('focused')) focused = false;
        },
      },
      scrollIntoView: () => { scrollCount += 1; },
      firstChild: null,
      parentNode: { insertBefore: () => {}, removeChild: () => { removedCount += 1; }, normalize: () => {} },
  ...
  const bridgeDocument: BridgeDocument = {
    ...
    createTreeWalker: () => {
      ...
      return {
        ...
        nextNode: () => { if (yielded) return false; yielded = true; return true; },
      };
    },
...
describe('BRIDGE_SCRIPT inbound message validation', () => {
  ...
  test('ignores malformed and unknown messages without changing runtime state', () => {
    ...
    for (const message of malformedMessages) runtime.bridgeWindow.dispatchMessage(message, runtime.parent);
  ...
  test('handles every mark variant, unknown messages, malformed values, and defaults', () => {
    ...
    const messages: any[] = [
      ...
      { type: 'plannotator-bridge-find-and-mark', id: 'find-deletion', originalText: 'text', annotationType: 'deletion' },
      { type: 'plannotator-bridge-find-and-mark', id: 'find-invalid-type', originalText: 'text', annotationType: {} },
    ...
    bridgeWindow.dispatchMessage({ type: 'plannotator-bridge-set-input-method', method: 'mouse' }, parent);
