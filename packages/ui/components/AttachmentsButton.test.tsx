...
async function renderAttachments(
  ...
  await act(async () => {
    root.render(<AttachmentsButton images={images} onAdd={onAdd} onRemove={onRemove} />);
  });
}
...
describe("AttachmentsButton upload response handling", () => {
  ...
  test.skipIf(!hasDom)(
    "does not call attachment callbacks for a malformed response body",
    async () => {
      const added: ImageAttachment[] = [];
      const removed: string[] = [];
  ...
  test.skipIf(!hasDom)(
    "does not call attachment callbacks for a non-OK error envelope",
    async () => {
      const added: ImageAttachment[] = [];
      const removed: string[] = [];
  ...
  test.skipIf(!hasDom)(
    "only removes an existing attachment after a valid re-edit upload",
    async () => {
      const added: ImageAttachment[] = [];
      const removed: string[] = [];
      installFetchResponses([imageResponse(), uploadResponse({ path: "/uploads/re-edited.png" })]);
  ...
  test.skipIf(!hasDom)(
    "keeps an existing attachment when a re-edit response is malformed",
    async () => {
      const added: ImageAttachment[] = [];
      const removed: string[] = [];
      installFetchResponses([imageResponse(), uploadResponse({ error: "Upload failed" }, 500)]);
