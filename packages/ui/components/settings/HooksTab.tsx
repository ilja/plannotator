        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
              Inspired by GitHub Flavored Markdown, PFM extends it with interactive tables, SVG
              diagrams (custom, Mermaid &amp; Graphviz), code-file links that open in your editor,
              callouts, task lists, and more. This reminder tells the planning agent what the
              renderer supports so it can use these features naturally.{" "}
              <strong>No extra tokens</strong> — the agent still writes markdown as it normally
              would, just with enhanced syntax that Plannotator renders richer.
            </p>
            <div className="mt-2">
              <span
                className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full ${
                  pfmEnabled ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"
                }`}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${pfmEnabled ? "bg-primary" : "bg-muted-foreground/50"}`}
                />
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${status.improvementHook.present ? "bg-primary" : "bg-muted-foreground/50"}`}
                />
            ...
            {status.improvementHook.present ? (
              <>
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                  ...
                  {status.improvementHook.fileSize != null && (
                    <span className="text-muted-foreground/70">
                      {" "}
                      · {(status.improvementHook.fileSize / 1024).toFixed(1)}KB
                    </span>
                  )}
                </p>
                ...
                <p className="text-[11px] text-muted-foreground/70 mt-2 leading-relaxed">
                  Edit this file directly to customize, or run{" "}
                  <code className="text-[10px] bg-muted px-1 py-0.5 rounded">
                    /plannotator-compound
                  </code>{" "}
                  to regenerate from recent denial history.
                </p>
                <button
              ...
              <>
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                  ...
                  before your agent writes a plan — the more you review, the better your agent
                  plans.
                </p>
                {status.improvementHook.filePath && (
                  <CopyPathButton filePath={status.improvementHook.filePath} />
                ...
                <p className="text-[11px] text-muted-foreground/70 mt-2 leading-relaxed">
                  ...
                  to auto-generate from your denial history, or create a plain text file at the path
                  above with your own instructions.{" "}
                  <a
