...
describe('HooksTab hooks status response handling', () => {
  test.skipIf(!hasDom)('renders a complete valid response', async () => {
    ...
    expect(document.body.textContent).toContain('~/.plannotator/hooks/compound/enterplanmode-improve-hook.txt');
  ...
  test.skipIf(!hasDom)('retains a valid PFM section when the improvement hook is malformed', async () => {
    installHooksStatusFetch({
      pfmReminder: { enabled: true },
