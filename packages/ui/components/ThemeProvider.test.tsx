...
function ThemeStateProbe({ onState }: { onState: (state: { mode: string; colorTheme: string }) => void }) {
  ...
  await act(async () => {
    root.render(
      <ThemeProvider
        ...
        colorThemeStorageKey={COLOR_STORAGE_KEY}
      >
        <ThemeStateProbe onState={(nextState) => { state = nextState; }} />
      </ThemeProvider>,
    );
  });
...
describe('ThemeProvider stored settings', () => {
  ...
  test.skipIf(!hasDom)('falls back independently for malformed stored values without rewriting them', async () => {
  ...
  test.skipIf(!hasDom)('uses the mode default without changing a valid stored color theme', async () => {
