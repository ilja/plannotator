...
function HookHarness({ resetKey, appliedCwds }: HookHarnessProps): React.JSX.Element {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={freshness.dismiss}>Dismiss</button>
    </div>
  );
}

async function mountHarness(resetKey: string, appliedCwds: Array<string | null>): Promise<{ host: HTMLDivElement; root: Root }> {
...
describe('useDiffFreshness response handling', () => {
  test.skipIf(!hasDom)('applies valid stale and fresh responses', async () => {
    installManualTimers();
    installFetch([
      new Response(JSON.stringify({ fresh: false, fingerprint: 'abc123', agentCwd: '/tmp/review' })),
  ...
  test.skipIf(!hasDom)('does not apply a cancelled poll after a reset', async () => {
    ...
    resolveFirst(new Response(JSON.stringify({ fresh: false, fingerprint: 'old', agentCwd: '/tmp/old' })));
