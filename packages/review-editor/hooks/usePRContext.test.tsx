...
function HookHarness(): React.JSX.Element {
  ...
  return (
    <div>
      <button type="button" onClick={() => void fetchContext()}>Fetch</button>
      <output data-loading={String(isLoading)} data-error={error ?? ''} data-context={prContext ? 'loaded' : ''}>
...
describe('readPRContextResponse', () => {
  test('resolves a valid successful response', async () => {
    await expect(readPRContextResponse(new Response(JSON.stringify(validContext), { status: 200 }))).resolves.toEqual(validContext);
  });

  test('rejects invalid JSON from a successful response', async () => {
    await expect(readPRContextResponse(new Response('{invalid-json', { status: 200 }))).rejects.toThrow();
  });

  test('rejects a malformed successful envelope', async () => {
    await expect(readPRContextResponse(new Response(JSON.stringify({ body: 'missing fields' }), { status: 200 }))).rejects.toThrow();
  });

  test('uses a string error from a non-OK response', async () => {
    await expect(readPRContextResponse(new Response(JSON.stringify({ error: 'Permission denied' }), { status: 403 }))).rejects.toThrow('Permission denied');
  });

  test('uses the HTTP status when a non-OK body is invalid JSON', async () => {
    await expect(readPRContextResponse(new Response('{invalid-json', { status: 502 }))).rejects.toThrow('HTTP 502');
  });

  test('uses the HTTP status when a non-OK error is not a string', async () => {
    await expect(readPRContextResponse(new Response(JSON.stringify({ error: { message: 'untrusted' } }), { status: 503 }))).rejects.toThrow('HTTP 503');
  });
});

describe('usePRContext response handling', () => {
  test.skipIf(!hasDom)('turns invalid successful JSON into an error and permits retry', async () => {
    installFetch([
  ...
  test.skipIf(!hasDom)('turns malformed successful envelopes into an error', async () => {
    installFetch([new Response(JSON.stringify({ body: 'missing required fields' }), { status: 200 })]);
    await renderHarness();
