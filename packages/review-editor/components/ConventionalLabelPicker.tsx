...
export function getEnabledLabels(configJson: string | null): LabelDef[] {
  ...
  try {
    ...
    return parsed.flatMap((cfg) => {
      // Malformed entries (wrong shape, non-string label/display) are skipped:
      // config is machine-generated by Settings.tsx, so a conforming entry is
      // the only shape that produces a useful picker button.
      if (
        cfg === null ||
        Array.isArray(cfg) ||
        Object(cfg) !== cfg ||
        !(cfg.label === String(cfg.label)) ||
        !(cfg.display === String(cfg.display))
      ) {
        return [];
      }
