...
export const PRCommentsTab: React.FC<PRCommentsTabProps> = React.memo(
  ({ context, platformUser }) => {
    ...
    const _hasFilters =
      !!searchQuery.trim() || excludedAuthors.size > 0 || hideResolved || hideOutdated;
                ...
                displayTimeline.map((entry) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!isCollapsed && (
                        <PRCommentLinkActions url={entry.data.url} body={entry.data.body} />
                      )}
                    </div>
                  );
                })
  ...
  return (
    <div
      ...
      className={`group/card rounded border p-3 cursor-pointer transition-all duration-150 ${
        isDimmed
          ? "bg-card/50 shadow-[0_1px_2px_rgba(0,0,0,0.03)]"
