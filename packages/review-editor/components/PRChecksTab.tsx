...
function CheckIcon({ check }: { check: PRCheck }) {
  if (check.status !== "COMPLETED") {
    ...
    return (
      <svg
        className="w-3.5 h-3.5 text-yellow-500 animate-spin"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-muted-foreground">
              ...
              {checkSummary.failed > 0 && (
                <span className="text-destructive ml-1">{checkSummary.failed} failed</span>
              )}
              {checkSummary.pending > 0 && (
                <span className="text-yellow-500 ml-1">{checkSummary.pending} pending</span>
              )}
            </span>
          </div>
                ...
                <div className="space-y-1">
                  {checks.map((check, i) => (
                    ...
                    >
                      ...
                      <svg
                        className="w-3 h-3 text-muted-foreground/30 ml-auto flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={2}
