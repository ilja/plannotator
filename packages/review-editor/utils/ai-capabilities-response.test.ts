...
describe("decodeReviewAICapabilitiesResponse", () => {
  ...
  it("normalizes malformed or missing default providers to null", () => {
    ...
    expect(
      decodeReviewAICapabilitiesResponse({ available: true, providers: validResponse.providers }),
    ).toEqual({
      ...validResponse,
      defaultProvider: null,
    });
...
describe("loadReviewAICapabilitiesState", () => {
  it("does not update for non-OK responses or invalid JSON", async () => {
    await expect(
      loadReviewAICapabilitiesState(new Response("not json", { status: 500 })),
    ).resolves.toBeUndefined();
    await expect(
      loadReviewAICapabilitiesState(new Response("{", { status: 200 })),
    ).resolves.toBeUndefined();
    await expect(
      loadReviewAICapabilitiesState(
        new Response(JSON.stringify({ available: "yes" }), { status: 200 }),
      ),
    ).resolves.toBeUndefined();
  });
  ...
  it("keeps only Pi providers and a matching default provider", async () => {
    const response = {
      ...validResponse,
      providers: [validProvider, { ...validProvider, id: "other", name: "other-provider" }],
      defaultProvider: "other",
    };

    await expect(
      loadReviewAICapabilitiesState(new Response(JSON.stringify(response))),
    ).resolves.toEqual({
      available: true,
      providers: [validProvider],
      defaultProvider: null,
