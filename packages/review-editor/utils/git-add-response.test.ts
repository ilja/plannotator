...
import {
  decodeGitAddResponse,
  readGitAddResponse,
} from './git-add-response';
...
describe('readGitAddResponse', () => {
  test('reads a valid successful response', async () => {
    await expect(readGitAddResponse(new Response(JSON.stringify({ ok: true })))).resolves.toEqual({ ok: true });
  });

  test('uses a string error from a non-OK response', async () => {
    await expect(readGitAddResponse(
