...
describe('decodePRContextResponse', () => {
  test('decodes GitHub-shaped context and preserves nullable and optional fields', () => {
    ...
    expect(decodePRContextResponse({
      ...validContext,
      reviews: [{ ...validReview, url: '' }],
    }).reviews).toEqual([{ ...validReview, url: '' }]);
  ...
  test('filters malformed top-level array entries while retaining valid siblings', () => {
    const decoded = decodePRContextResponse({
      ...
      checks: [validContext.checks[0], { ...validContext.checks[0], conclusion: 42 }, 'not-a-check'],
      linkedIssues: [validContext.linkedIssues[0], { ...validContext.linkedIssues[0], number: '7' }, 'not-an-issue'],
    });
  ...
  test('filters malformed thread comments without dropping valid threads or siblings', () => {
    const decoded = decodePRContextResponse({
      ...
      reviewThreads: [
        {
          ...validThread,
          comments: [validThread.comments[0], { ...validThread.comments[0], url: 42 }, 'not-a-comment'],
        },
        { ...validThread, path: 42 },
        'not-a-thread',
      ],
