...
import { decodePRActionResponse, readPRActionResponse } from "./pr-action-response";

describe("decodePRActionResponse", () => {
  test("decodes successful responses with optional and empty URLs", () => {
    expect(
      decodePRActionResponse({ ok: true, prUrl: "https://github.com/example/repo/pull/1" }),
    ).toEqual({
      ok: true,
...
describe("readPRActionResponse", () => {
  test("reads successful responses and preserves an empty URL", async () => {
    ...
    await expect(readPRActionResponse(new Response(JSON.stringify({ ok: true })))).resolves.toEqual(
      { ok: true },
    );
    await expect(
      readPRActionResponse(new Response(JSON.stringify({ ok: true, prUrl: "" }))),
    ).resolves.toEqual({
      ok: true,
      prUrl: "",
    });
  ...
  test("uses string errors for non-OK and 200 error envelopes", async () => {
    await expect(
      readPRActionResponse(
        new Response(JSON.stringify({ error: "Authentication failed" }), { status: 500 }),
      ),
    ).resolves.toEqual({
      ok: false,
      error: "Authentication failed",
    ...
    await expect(
      readPRActionResponse(new Response(JSON.stringify({ ok: false, error: "Target failed" }))),
    ).resolves.toEqual({
      ok: false,
      error: "Target failed",
    });
    await expect(
      readPRActionResponse(new Response(JSON.stringify({ error: "Target failed" }))),
    ).resolves.toEqual({
      ok: false,
      error: "Target failed",
    });
