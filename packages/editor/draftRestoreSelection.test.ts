...
function draft(
  key: string,
  scope: "single-file" | "folder-file" = "single-file",
...
describe("pickRestoredSingleFileDraftToDisplay", () => {
  ...
  test("shows the only restored single-file draft when nothing is active", () => {
    expect(pickRestoredSingleFileDraftToDisplay([draft("a")], ["a"], null)?.key).toBe("a");
  });

  test("does not guess across multiple drafts or folder files", () => {
    expect(
      pickRestoredSingleFileDraftToDisplay([draft("a"), draft("b")], ["a", "b"], null),
    ).toBeUndefined();
    expect(
      pickRestoredSingleFileDraftToDisplay([draft("a", "folder-file")], ["a"], null),
    ).toBeUndefined();
  });
});
