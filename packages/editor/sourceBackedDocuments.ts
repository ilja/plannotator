...
export type SourceBackedDocumentSaveStatus =
  | "clean"
  | "dirty"
  | "saving"
  | "saved"
...
function basenameForCapability(
  sourceSave: SourceSaveCapability | null,
  fallbackKey: string,
): string {
...
export function sourceBackedDocumentKey(
...
export function getSourceBackedDocumentKnownDiskHash(
...
export function markSourceBackedDocumentSaved(
  record: SourceBackedDocumentRecord,
  input: SourceBackedDocumentSaveInput,
): void {
...
export function useSourceBackedDocuments(options: SourceBackedDocumentLifecycleOptions = {}) {
  ...
  const openSourceBackedDocument = useCallback(
    ({ key, text, sourceSave }: OpenSourceBackedDocumentInput) => {
  ...
  const discardSourceBackedDocument = useCallback(
    (key: string): SourceBackedDocumentRecord | null => {
      ...
      record.saveStatus = record.diskConflict
        ? cleanOrDirty(record)
        : record.savedChange
          ? "saved"
          : "clean";
  ...
  const reconcileDiskSnapshot = useCallback(
    (
      input: SourceBackedDocumentDiskSnapshotInput,
    ): SourceBackedDocumentDiskSnapshotReconcileResult => {
      const result = reconcileSourceBackedDocumentDiskSnapshot(
      ...
      return result.type === "missing"
        ...
        : ({
          ...
          } as SourceBackedDocumentDiskSnapshotReconcileResult);
    },
    [bump],
  );
  ...
  const reloadSourceBackedDocument = useCallback(
  ...
  const discardSourceBackedDocumentEdits = useCallback(
    (key: string): SourceBackedDocumentLifecycleOutcome => {
      const before = docsRef.current.get(key)?.currentText;
  ...
  const restoreSourceBackedDraftDocuments = useCallback(
    (documents: SourceBackedDocumentDraftData[]): string[] => {
      ...
      for (const doc of documents) {
        ...
        const restored: SourceBackedDocumentRecord = {
          ...
          saveStatus: doc.missingOnDisk
            ? "missing"
            : currentText === diskBaseline
              ? "clean"
              : "dirty",
  ...
  const restoreSourceBackedSavedFileChanges = useCallback(
    (changes: SourceBackedSavedFileChangeDraftData[]) => {
  ...
  const validateSourceBackedSavedFileChangesForLifecycle = useCallback(
    async (
      changes: SourceBackedSavedFileChangeDraftData[],
    ): Promise<SourceBackedSavedFileChangeValidationResult> => {
      return validateSourceBackedSavedFileChanges(changes, (change) =>
        probeSourceSaveAdapter(change.path),
      );
    },
    [probeSourceSaveAdapter],
  );

  const markSourceBackedDocumentMissing = useCallback(
    (
      key: string,
    ): {
      record: SourceBackedDocumentRecord;
  ...
  const reconcileSourceBackedDocuments = useCallback(
    async (changedDir?: string): Promise<SourceBackedDocumentLifecycleOutcome[]> => {
  ...
  const saveSourceBackedDocument = useCallback(
    async (
      ...
      return {
        ...
        reason:
          saveResult.status === "conflict-incomplete"
    ...
    return Array.from(docsRef.current.values())
      ...
      .map((record) => {
        const draft: SourceBackedDocumentDraftData = {
          ...
          savedChange: record.savedChange
  ...
  const getSourceBackedDraftSavedFileChanges =
    useCallback((): SourceBackedSavedFileChangeDraftData[] => {
      return Array.from(docsRef.current.values())
        .filter(
          (
            record,
          ): record is SourceBackedDocumentRecord & {
            sourceSave: EnabledSourceSaveCapability;
            savedChange: SourceBackedSavedFileChange;
          } =>
  ...
  const getSourceBackedSavedFileChangesForValidation =
    useCallback((): SourceBackedSavedFileChangeDraftData[] => {
      return Array.from(docsRef.current.values())
        .filter(
          (
  ...
  const getSourceBackedDocuments = useCallback((): Array<
    SourceBackedDocumentRecord & { sourceSave: EnabledSourceSaveCapability }
  > => {
    return Array.from(docsRef.current.values())
      .filter(
        (
