...
describe("direct edit feedback helpers", () => {
  ...
  test("replaces empty feedback with direct edits instead of appending the sentinel", () => {
    ...
    expect(
      composeFeedbackWithDirectEdits("User reviewed the document and has no feedback.", edits),
    ).toBe(edits);
    expect(composeFeedbackWithDirectEdits("A real annotation.", edits)).toBe(
      `${edits}\n\n---\n\nA real annotation.`,
    );
  });
  ...
  test("includes saved file changes even when they are the only feedback", () => {
    ...
    expect(
      composeFeedbackWithEditSections("User reviewed the document and has no feedback.", "", saved),
    ).toBe(saved);
    expect(composeFeedbackWithEditSections("Please adjust the intro.", "", saved)).toBe(
