...
async function mountSourceBackedDocuments(
  options: SourceBackedDocumentLifecycleOptions = {},
): Promise<{
...
describe("useSourceBackedDocuments lifecycle actions", () => {
  ...
  test.skipIf(!hasDom)("document snapshots do not mutate collection state", async () => {
    ...
    await act(async () => {
      session
        .current()
        .openSourceBackedDocument({ key: KEY, text: "a\n", sourceSave: initialSource });
      session.current().updateSourceBackedDocumentText(KEY, "local\n");
  ...
  test.skipIf(!hasDom)(
    "keeps stable keyed snapshots while selection moves outside the collection",
    async () => {
      const session = await mountSourceBackedDocuments();
      ...
      await act(async () => {
        ...
        session
          .current()
          .openSourceBackedDocument({ key: secondKey, text: "b\n", sourceSave: SOURCE_B });
      });
...
describe("useSourceBackedDocuments conflict actions", () => {
  test.skipIf(!hasDom)(
    "overwrite conflict records the diff from the latest disk version",
    async () => {
      const session = await mountSourceBackedDocuments({
        readSourceDocument: async () => ({
          status: "ok",
      ...
      expect(session.current().getSourceBackedDocument(KEY)?.diskConflict?.sourceSave.hash).toBe(
        "sha256:external",
      );
  ...
  test.skipIf(!hasDom)(
    "dirty saved-then-edited drafts keep saved context nested without duplicating it",
    async () => {
      const session = await mountSourceBackedDocuments({
  ...
  test.skipIf(!hasDom)(
    "restoring draft documents does not overwrite a live dirty buffer",
    async () => {
      const session = await mountSourceBackedDocuments();
  ...
  test.skipIf(!hasDom)(
    "missing source files keep the buffer and clear stale saved context",
    async () => {
      const session = await mountSourceBackedDocuments({
  ...
  test.skipIf(!hasDom)(
    "discarding a missing source file removes the in-memory buffer",
    async () => {
      const session = await mountSourceBackedDocuments({
        readSourceDocument: async () => ({ status: "missing" }),
      });
...
describe("useSourceBackedDocuments lifecycle commands", () => {
  test.skipIf(!hasDom)("validates saved changes through the lifecycle probe adapter", async () => {
    const change = (
      key: string,
      path: string,
      overrides: Partial<SourceBackedSavedFileChangeDraftData> = {},
    ...
    await act(async () => {
      result = await session
        .current()
        .validateSourceBackedSavedFileChanges([fresh, stale, missing, noop, unavailable]);
    });
  ...
  test.skipIf(!hasDom)("returns applied, unavailable, and ignored read outcomes", async () => {
    ...
    await act(async () => {
      newerOutcomes = await newer;
    });
    firstRead.resolve({ status: "ok", snapshot: { markdown: "old\n", sourceSave: SOURCE_B } });
    let olderOutcomes: Awaited<typeof older> = [];
    await act(async () => {
      olderOutcomes = await older;
    });

    expect(newerOutcomes).toEqual([
      expect.objectContaining({ type: "disk-update-applied", previousText: "a\n" }),
    ]);
    expect(olderOutcomes).toEqual([
      expect.objectContaining({ type: "disk-observation-ignored", reason: "stale-sequence" }),
    ]);
    ...
    await act(async () => {
      unavailableOutcomes = await unavailable;
    });
    ...
    await act(async () => {
      staleOutcomes = await staleReconcile;
    });
  ...
  test.skipIf(!hasDom)(
    "saves through the adapter and applies complete conflict snapshots",
    async () => {
      ...
      await act(async () => {
        conflict = await session
          .current()
          .saveSourceBackedDocument({ key: KEY, text: "overwrite\n" });
      });
      expect(conflict).toEqual(expect.objectContaining({ type: "save-conflict" }));
      expect(session.current().getSourceBackedDocument(KEY)?.diskConflict).toEqual(
        expect.objectContaining({ text: "external\n" }),
      );
      ...
      expect(saveRequests.at(-1)).toEqual(
        expect.objectContaining({
          baseHash: "sha256:external",
          text: "overwrite\n",
        }),
      );
      expect(session.current().getSourceBackedDocument(KEY)?.savedChange).toEqual(
        expect.objectContaining({
          beforeText: "external\n",
          beforeHash: "sha256:external",
          afterText: "overwrite\n",
  ...
  test.skipIf(!hasDom)(
    "fails closed for incomplete conflict snapshots and write failures",
    async () => {
      let saveResult: Awaited<
        ReturnType<NonNullable<SourceBackedDocumentLifecycleOptions["saveSourceDocument"]>>
      > = {
      ...
      await act(async () => {
        incomplete = await session
          .current()
          .saveSourceBackedDocument({ key: KEY, text: "local\n" });
      });
      expect(incomplete).toEqual(
        expect.objectContaining({ type: "save-error", reason: "conflict-snapshot-unavailable" }),
      );
      ...
      await act(async () => {
        failed = await session.current().saveSourceBackedDocument({ key: KEY, text: "local\n" });
      });
  ...
  test.skipIf(!hasDom)(
    "reloads a clean buffer when Save reports a complete conflict snapshot",
    async () => {
      const session = await mountSourceBackedDocuments({
      ...
      await act(async () => {
        outcome = await session.current().saveSourceBackedDocument({ key: KEY, text: "a\n" });
      });
      expect(outcome).toEqual(
        expect.objectContaining({ type: "save-disk-update-applied", previousText: "a\n" }),
      );
      expect(session.current().getSourceBackedDocument(KEY)?.currentText).toBe("external\n");
      expect(session.current().getSourceBackedDocument(KEY)?.diskConflict).toBeUndefined();
  ...
  test.skipIf(!hasDom)("reloads and discards through lifecycle outcomes", async () => {
    ...
    expect(reloaded).toEqual(
      expect.objectContaining({ type: "document-reloaded", previousText: "local\n" }),
    );
    expect(session.current().getSourceBackedDocument(KEY)?.currentText).toBe("external\n");
    ...
    expect(discarded).toEqual(
      expect.objectContaining({ type: "document-discarded", previousText: "local again\n" }),
    );
    expect(session.current().getSourceBackedDocument(KEY)?.currentText).toBe("external\n");
