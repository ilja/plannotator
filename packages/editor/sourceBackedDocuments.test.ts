...
describe("sourceBackedLinkedDocumentKey", () => {
  test("enrolls enabled linked documents in the lifecycle collection", () => {
    expect(sourceBackedLinkedDocumentKey(sourceSave("sha256:after"), "/repo/docs/a.md")).toBe(
      "file:/repo/docs/a.md",
    );
  });
  ...
  test("keeps absent or disabled linked documents on the markdown cache path", () => {
    ...
    expect(
      sourceBackedLinkedDocumentKey(disabledSourceSave("unsupported-extension"), "/repo/docs/a.md"),
    ).toBeNull();
  });
});
...
describe("reconcileSourceBackedDocumentDiskSnapshot", () => {
  ...
  test("disk snapshots only apply to the record version that requested them", () => {
    ...
    expect(
      canApplySourceBackedDocumentDiskSnapshot({ ...doc, saveStatus: "saving" }, "sha256:after"),
    ).toBe(false);
...
describe("canRestoreSourceBackedDocumentDraft", () => {
  ...
  test("does not restore over newer session state", () => {
    ...
    expect(
      canRestoreSourceBackedDocumentDraft(
        { ...cleanDoc, saveStatus: "missing", missingOnDisk: true },
        source,
        "after\n",
      ),
    ).toBe(false);
    expect(
      canRestoreSourceBackedDocumentDraft(
        { ...cleanDoc, saveStatus: "saved", savedChange: record().savedChange },
        source,
        "after\n",
      ),
    ).toBe(false);
    expect(
      canRestoreSourceBackedDocumentDraft(
        { ...cleanDoc, sourceSave: sourceSave("sha256:external") },
        source,
        "after\n",
      ),
    ).toBe(false);
