...
describe("source-save node helpers", () => {
  ...
  test.skipIf(process.platform === "win32")(
    "does not replace an occupied missing path while recreating a file",
    () => {
      const root = tempRoot();
      const filePath = join(root, "plan.md");
  ...
  test.skipIf(process.platform === "win32")(
    "does not treat an unreadable existing file as missing",
    () => {
      const root = tempRoot();
      const filePath = join(root, "plan.md");
      writeFileSync(filePath, "Before\n");
  ...
  test("resolves missing folder source leaves without following symlinked parents outside the folder", () => {
    ...
    expect(resolveFolderSourceFileForSave(join(folder, "new.md"), folder)).toBe(
      join(realpathSync(folder), "new.md"),
    );
