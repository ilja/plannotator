...
export function getGitRepositoryInfo(cwd: string): GitRepositoryInfo | null {
  ...
  return {
    ...
    gitCommonDir:
      gitCommonDir.ok && gitCommonDir.stdout.trim()
...
async function computeWorkspaceStatusForDirectory(
  rootPath: string,
...
async function runWorkspaceStatusFlight(
  rootPath: string,
  flight: WorkspaceStatusFlight,
): Promise<WorkspaceStatusPayload> {
  try {
...
export async function getWorkspaceStatusForDirectory(
  dirPath: string,
): Promise<WorkspaceStatusPayload> {
  let rootPath: string;
