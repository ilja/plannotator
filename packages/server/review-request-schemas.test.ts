...
describe("review request schemas", () => {
	test("DiffTypeSchema accepts VCS types and rejects workspace types", () => {
		...
		expect(Option.getOrUndefined(Schema.decodeUnknownOption(DiffTypeSchema)("worktree:feature"))).toBe(
			"worktree:feature",
		);
		expect(Option.getOrUndefined(Schema.decodeUnknownOption(DiffTypeSchema)("p4-changelist:123"))).toBe(
			"p4-changelist:123",
		);
		expect(
	...
	test("WorkspaceDiffTypeSchema accepts workspace variants only", () => {
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(WorkspaceDiffTypeSchema)("workspace-current")),
	...
	test("DiffSwitchRequestSchema validates diffType and optional fields", () => {
		...
		expect(
			Option.getOrUndefined(
				Schema.decodeUnknownOption(DiffSwitchRequestSchema)({ diffType: "uncommitted", hideWhitespace: "true" }),
			),
		).toBeUndefined();
	});
	...
	test("PrDiffScopeRequestSchema validates layer and full-stack", () => {
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(PrDiffScopeRequestSchema)({ scope: "layer" }))?.scope,
		).toBe("layer");
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(PrDiffScopeRequestSchema)({ scope: "full-stack" }))
				?.scope,
		).toBe("full-stack");
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(PrDiffScopeRequestSchema)({ scope: "invalid" })),
		).toBeUndefined();
		expect(
	...
	test("PrSwitchRequestSchema requires non-empty url", () => {
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(PrSwitchRequestSchema)({ url: "https://github.com/org/repo/pull/1" }))
				?.url,
	...
	test("GitAddRequestSchema validates filePath and optional undo", () => {
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(GitAddRequestSchema)({ filePath: "src/app.ts" }))
				?.filePath,
		).toBe("src/app.ts");
		expect(
	...
	test("FeedbackRequestSchema validates optional fields", () => {
		...
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(FeedbackRequestSchema)({ approved: "true" })),
		).toBeUndefined();
	});
	...
	test("PrActionRequestSchema requires action, body and fileComments", () => {
		...
		expect(
			Option.getOrUndefined(
				Schema.decodeUnknownOption(PrActionRequestSchema)({ action: "invalid", body: "hi", fileComments: [] }),
			),
		).toBeUndefined();
	...
	test("PrViewedRequestSchema validates filePaths and viewed", () => {
		...
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(PrViewedRequestSchema)({ filePaths: "a.ts", viewed: true })),
		).toBeUndefined();
		expect(
			Option.getOrUndefined(Schema.decodeUnknownOption(PrViewedRequestSchema)({ filePaths: [], viewed: "true" })),
		).toBeUndefined();
		expect(
