...
describe("Pi file browser watcher", () => {
  test("ignores nested excluded folders for watcher paths", () => {
    ...
    expect(isFileBrowserWatchIgnoredPath(join(root, "packages", "app", "node_modules"), root)).toBe(
      true,
    );
    expect(
      isFileBrowserWatchIgnoredPath(
        join(root, "packages", "app", "node_modules", "pkg", "readme.md"),
        root,
      ),
    ).toBe(true);
    expect(isFileBrowserWatchIgnoredPath(join(root, "docs", "dist", "generated.md"), root)).toBe(
      true,
    );
    ...
    expect(
      isFileBrowserWatchIgnoredPath(join(dirname(root), "outside", "node_modules"), root),
    ).toBe(false);
  });
});
