...
function safeNotify(
  ...
  try {
    ...
    console.error(
      `Plannotator notification failed: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
      ...
      try {
        ...
        void session
          ...
          .catch((err) => {
            reportBackgroundError(
              ctx,
              "Plannotator code review session failed",
              err instanceof Error ? err : new Error(String(err)),
              origin,
            );
          });
      ...
      try {
        ...
        void session
          ...
          .catch((err) => {
            reportBackgroundError(
              ctx,
              "Plannotator annotation session failed",
              err instanceof Error ? err : new Error(String(err)),
              origin,
            );
          });
